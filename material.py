from gaussian6d import Gaussian6D
import taichi as ti
from vector import *
...
class Metal(_material):
    ...
    def scatter(in_direction, p, n, color, roughness):
        out_direction = (
            reflect(in_direction.normalized(), n) + roughness * random_in_unit_sphere()
        )
        attenuation = color
...
class Materials:
    ...
    def __init__(self, n):
        ...
        ti.root.dense(ti.i, n).place(
            self.roughness, self.colors, self.mat_index, self.ior
        )
        self.gaussian = [Gaussian6D() for _ in range(n)]
    ...
    def scatter(self, i, ray_direction, p, n, front_facing):
        ...
        if mat_index == 0:
            ...
            reflected, out_origin, out_direction, attenuation = self.gaussian[
                0
            ].scatter(ray_direction, p, n, color)
        else:
            print(f"Material index {mat_index} not found")
            reflected, out_origin, out_direction, attenuation = Lambert.scatter(
                ray_direction, p, n, color
            )
